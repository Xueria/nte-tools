...
func (page *InferPage) inferQuery() (bid.InferQuery, bool) {
	minCount, maxCount := page.countRange()

	if page.modeRadio.Selected == priceModeTotal {
		total, ok := parseNonNegativeInt(page.totalEntry.Text)
	...
	avg, ok := parseNonNegativeInt(page.avgEntry.Text)
	...
	if strings.TrimSpace(page.maxTotalEntry.Text) != "" {
		maxTotal, ok = parseNonNegativeInt(page.maxTotalEntry.Text)
...
func (page *InferPage) countRange() (minCount, maxCount int) {
